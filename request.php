...
if($t == 'url_preview'){
    $parsedUrl = $_POST['url'];
    $apikey = 'bf5c3cd37105400480e241719b0b7785';
    $apiurl = 'http://api.embed.ly/1/extract?url=%s&maxwidth=%d&key=%s';
    $imgWidth = $_POST['size'];
    $url_format = sprintf($apiurl,$parsedUrl, $imgWidth, $apikey);
    $json = file_get_contents($url_format);
    $obj = json_decode($json);
    $html_img = "";
    foreach($obj->images as $i){
        if(!empty($i->url)){
            $html_img .= '<img src="'.$i->url.'" width="100%" title="'.$obj->title.'" alt="'.$obj->title.'"/>';
            break;
        }
    }
    $html = SK_getPage('story/publisher-box/urlPreview');
    if (!empty($html)) {
        $data = array(
            'status' => 200,
            'type' => 'api',
...
if($t == 'fUser') {
    $q = $_GET['q'];
    $data = array();
    $query = "SELECT id, name FROM " . DB_ACCOUNTS . " WHERE name like '%".$q."%' AND type='user' AND active=1";
    $q_result = mysqli_query($dbConnect, $query);
    while($obj = mysqli_fetch_object($q_result)) {
        $data[] = SK_getUser($obj->id);
    }
    echo json_encode($data);
    mysqli_close($dbConnect);
    exit();
