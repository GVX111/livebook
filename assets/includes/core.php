...
function SK_getStory($story_id=0, $view_all_comments=false) {
    ...
    $query_one = "SELECT id,active,activity_text,google_map_name,hidden,link_title,link_url,media_id,post_id,recipient_id,soundcloud_title,soundcloud_uri,text,PreviewData,time,timeline_id,type1,type2,youtube_video_id,youtube_title FROM " . DB_POSTS . " WHERE id=$story_id AND type1='story' AND active=1";
    $sql_query_one = mysqli_query($dbConnect, $query_one);
    ...
    if (mysqli_num_rows($sql_query_one) == 1) {
        ...
        if ($post['id'] == $post['post_id']) { 
            ...
            $query_two = "SELECT id,active,activity_text,google_map_name,hidden,link_title,link_url,media_id,post_id,recipient_id,soundcloud_title,soundcloud_uri,text,PreviewData,time,timeline_id,type1,type2,youtube_video_id,youtube_title FROM " . DB_POSTS . " WHERE id=" . $post['post_id'] . " AND type1='story' AND type2='none' AND active=1";
            $sql_query_two = mysqli_query($dbConnect, $query_two);
        ...
        $story['PreviewData'] = html_entity_decode( html_entity_decode($story['PreviewData']));
...
function SK_registerPost($data=array()) {
    ...
    if (isset($data['photos']['name']) || isset($data['videos']['name'])) {
        $isVideo = isset($data['videos']['name']) ? true : false;
        $contentType = isset($data['videos']['name']) ? 'videos' : 'photos';
        if (count($data[$contentType]['name']) == 1) {
            $params = array(
                'tmp_name' => $data[$contentType]['tmp_name'][0],
                'name' => $data[$contentType]['name'][0],
                'size' => $data[$contentType]['size'][0]
            );
            $photo_data = SK_registerMedia($params, 0, $isVideo);
            ...
            if ($sql_query_one) {
                ...
                for ($i = 0; $i < count($data[$contentType]['name']); $i++) {
                    $photo_param = array(
                        'tmp_name' => $data[$contentType]['tmp_name'][$i],
                        'name' => $data[$contentType]['name'][$i],
                        'size' => $data[$contentType]['size'][$i]
                    );
    ...
    if ($post_ability == true) {
        $query_one = "INSERT INTO " . DB_POSTS . " (active,google_map_name,media_id,soundcloud_title,soundcloud_uri,text,PreviewData, time,timeline_id,recipient_id,type1,type2,youtube_video_id,youtube_title) VALUES (1,'$google_map_name',$media_id,'$soundcloud_title','$soundcloud_uri','$text','$previewData'," . time() . "," . $timeline['id'] . ",$recipient_id,'$type1','$type2','$youtube_video_id','$youtube_title')";
        $sql_query_one = mysqli_query($dbConnect, $query_one);
...
function SK_registerMedia($upload, $album_id=0, $video = false) {
    if ($GLOBALS['logged'] !== true) {
        return false;
    }
    global $dbConnect;
    set_time_limit(0);
    $folder_name = $video ? 'videos' : 'photos';
    $type = $video ? 'video' : 'photo';

    if (!file_exists($folder_name . '/'  . date('Y'))) {
        mkdir($folder_name . '/' . date('Y'), 0777, true);
    }
    
    if (!file_exists($folder_name . '/'  . date('Y') . '/' . date('m'))) {
        mkdir($folder_name . '/' . date('Y') . '/' . date('m'), 0777, true);
    }
    
    $dir = $folder_name . '/' . date('Y') . '/' . date('m');
    $cover_image_id = 0;
    if (is_uploaded_file($upload['tmp_name'])) {
        ...
        if ($upload['size'] > 1024) {
            ...
            if (preg_match('/(jpg|jpeg|png|avi|wma|wmv|mp4|flv)/', $ext)) {

                $query_one = "INSERT INTO " . DB_MEDIA . " (extension,name,type) VALUES ('$ext','$name','$type')";
                $sql_query_one = mysqli_query($dbConnect, $query_one);
                
                if ($sql_query_one) {
                    $sql_id = mysqli_insert_id($dbConnect);
                    $uId = SK_generateKey() . '_' . $sql_id . '_' . md5($sql_id);
                    $original_file_name = $dir . '/' . $uId;
                    $original_file = $original_file_name . '.' . $ext;
                    $original_file_temp = $original_file_name . ($video ? '_temp.' : '.') . $ext;

                    if (move_uploaded_file($upload['tmp_name'], $original_file_temp)) {
                        if($video){
                            include 'convert/index.php';
                            $thread = new ConvertVideo($original_file_temp, $original_file_name.'.mp4', $original_file_name.'.jpg');
                            $thread->run();
                            unlink($original_file_temp);
                            list($width, $height) = getimagesize($original_file_name .'.jpg');
                            mysqli_query($dbConnect, "INSERT INTO " . DB_MEDIA . " (extension,name,type) VALUES ('jpg','$uId','photo')");
                            $cover_image_id = mysqli_insert_id($dbConnect);
                            $tempHist = $sql_id;
                            $sql_id = $cover_image_id;
                            $cover_image_id = $tempHist;
                        }else{
                            list($width, $height) = getimagesize($original_file_temp);
                        }
                        ...
                        $sizes = $video ? $video_size : $imageSizes;
                        foreach ($sizes as $ratio => $data) {
                            $save_file = $data['name'] . '.' . ($video ? 'jpg' : $ext);
                            SK_processMedia($data['type'], ($video ? "$original_file_name.jpg" : $original_file), $save_file, $data['width'], $data['height']);
                        }
                        ...
                        if($cover_image_id > 0)
                            mysqli_query($dbConnect, "UPDATE " . DB_MEDIA . " SET album_id=$album_id,url='$original_file_name',temp=0,active=1 WHERE id=$cover_image_id");
