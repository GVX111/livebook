...
$(function () {
    ...
    $(document).on('keyup', '.auto-grow-input', function(e) {
        var elem = $(this);
        var closest = elem.closest('form')
		...
		if(isUrl(elem.val()) && !sendingData){
			sendingData = true;
            closest.find('.box-header-wrapper').append('<div class="lineloader float-right"></div>');
			$.post(SK_source() + '?t=url_preview', {url: elem.val(), size: 600}, function (data) {
				if (data.status == 200) {
                    closest.find('.lineloader').remove();
					elem.parent().find('#datapreview').remove();
					elem.after(data.html);
					var html_preview = data.html;
					elem.after('<input type="hidden" id="previewContent" name="content"/>');
					elem.parent().find('#previewContent').val(html_preview.escape())
				}
			});
		}
        $(".123").tokenInput(SK_source() + '?t=fUser', {
            theme: 'facebook',
            ...
            resultsFormatter: function(item){
                return "<li>" + "<img src='" + item.avatar_url + "' title='" + item.name + "' height='32px' width='32px' />" +
                       "<div style='display: inline-block; padding-left: 10px;'>" + item.name  + "</li>" }
        });
    });
});
